...
/* Several R36S/OGA firmwares expose the built-in pad as GO-Super Gamepad but
 * ship an SDL mapping that stops at the D-pad.  Face buttons work while
 * BACK/START/L3/R3 silently stay disconnected.  Build the entry from the GUID
 * reported by the firmware (new SDL versions embed a CRC in that GUID), so the
 * fix works with both old and new controller databases without replacing a
 * user's explicit TER_GP_MAP override. */
static void np_add_go_super_mapping(int index) {
  ...
  fprintf(stderr,
          "[NATPAD] GO-Super mapping guid=%s back=b12 start=b13 "
          "l3=b14 r3=b15 (%s)\n",
          guid_string, result >= 0 ? "ok" : SDL_GetError());
}
...
static void np_poll_sdl(void) {
  ...
  if (!tried) {
    ...
    if (!um || !*um)
      for (int i = 0; i < n; i++) np_add_go_super_mapping(i);
    for (int i = 0; i < n; i++) {
